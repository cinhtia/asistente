		...
		<div class="card-body">
			<div class="row">
				<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
					<button class="btn btn-primary" id="btn_nuevo_item" data-toggle="tooltip" data-placemente="top" title="Nueva habilidad">
				...
				<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
					<form class="form-inline pull-right" id="form_search">
					  <div class="form-group mb-2">
					    <label for="descrip_habilidad_buscar" >Nombre &nbsp;</label>
					    <input type="text" class="form-control" name="descrip_habilidad_buscar" id="descrip_habilidad_buscar" placeholder="nombre">
					  </div>&nbsp;
...
<script type="text/javascript">
	...
	var cargarListado = function(page){
		...
		var descrip = $('#descrip_habilidad_buscar').val();
		if(descrip.trim() != ""){
			data.descrip_habilidad = descrip;
		}
		$.ajax({
		    url: '<?= ruta("habilidad.listado") ?>',
		    method: 'get',
		    data: data,
	...
	$(document).off('click','.btn-editar-item').on('click','.btn-editar-item', function (e){
		...
		utils.modal.remote({
			url: '<?= ruta("habilidad.editar") ?>',
			data: {id_compo_habilidad: id},
			modal_options: {
				titulo: 'Editar',
			},
			error: function(){
	...
	$(document).off('click','.btn-eliminar-item').on('click','.btn-eliminar-item', function (e){
		...
		utils.modal.confirm({
			titulo: 'Confirmación',
			contenido: '¿Confirma que desea eliminar esta habilidad?',
			type: 'danger',
			size: 'md',
